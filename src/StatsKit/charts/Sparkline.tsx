...
function toT(view: Viewing, fallbackIndex: number) {
  const ts = Date.parse(view.started_at || view.date || view.created_at || "") || 0;
  return ts || fallbackIndex; // ordine stabile anche senza date
}

/* ------------------------------------------------------------------ */
/*  Serie pronte (retro-compatibili)                                  */
/* ------------------------------------------------------------------ */
...
export function buildVotesGiven(history: Viewing[], user: string) {
  const items = history
    .map((h, i) => {
      ...
      return { t: toT(h, i), val: v, title: h.movie?.title };
    })
    .filter(Boolean) as { t: number; val: number; title?: string }[];

  // reindex per evitare buchi temporali
  return items.sort((a, b) => a.t - b.t).map((p, i) => ({ ...p, t: i }));
}
...
type BaseProps = {
  ...
  zeroLine?: boolean;
  /** Forza il dominio Y: [min,max] o "auto". Di default: avg→[4,10], delta→auto, percent→[0,100] */
  yDomain?: [number, number] | "auto";
  /** "avg" (4..10), "delta" (auto) oppure "percent" (0..100) */
  mode?: Mode;
...
export function Sparkline({
  ...
  const [hover, setHover] = React.useState<null | {
    boxX: number;
    boxY: number;
    d: { x: number; y: number; val: number; title?: string; label?: string };
  }>(null);
  ...
  React.useEffect(() => {
    ...
    const ro = new ResizeObserver((es) =>
      setW(Math.max(260, Math.floor(es[0].contentRect.width)))
    );
  ...
  const norm = (data || [])
    .map((p) => ({
      t: p.t,
      val: Number.isFinite(p.val) ? (p.val as number) : (p.avg as number),
      title: p.title,
      label: p.label,
    }))
  ...
  if (mode === "percent" && (!yDomain || yDomain === "auto")) {
    minY = 0;
    maxY = 100;
  } else if (yDomain && yDomain !== "auto") {
    [minY, maxY] = yDomain;
  } else if (mode === "avg") {
    minY = 4;
    maxY = 10;
  ...
  // --- dimensioni canvas + padding interno orizzontale ---
  const H = height;
  const X_PAD = 10; // padding extra a sinistra/destra per non attaccare al bordo
  const innerW = W - padding * 2 - X_PAD * 2;
  ...
  const ny = (y: number) =>
    padding + (1 - (y - minY) / (maxY - minY || 1)) * innerH;

  // proietta e ordina per x
  const pts = norm
    .map((p) => ({ ...p, x: nx(p.t), y: ny(p.val as number) }))
    .sort((a, b) => a.x - b.x);
  ...
  return (
    <div ref={ref} className="relative w-full">
      <svg width={W} height={H} className="block">
        ...
        {/* area + linea */}
        <path
          d={`${d} L ${baseRight},${H - padding} L ${baseLeft},${H - padding} Z`}
          fill={`url(#area-${gid})`}
          pointerEvents="none"
        />
        <path
        ...
        {/* zero line opzionale / di default per delta */}
        {(zeroLine ?? mode === "delta") && (
          <line
            x1={baseLeft}
            x2={baseRight}
        ...
        {/* etichette Y per "delta" se yDomain fissato */}
      ...
      {hover && (
        ...
        >
          ...
          {mode === "delta" ? (
            ...
            <div className="mt-1">{hover.d.val}%</div>
          ) : (
            <div className="mt-1">{formatScore(hover.d.val)}</div>
          )}
        </div>
      )}
    </div>
...
export function SparklineClassic({
  ...
  data: Array<{ t: number; avg: number }>;
  height?: number;
  padding?: number;
}) {
  return (
    <Sparkline mode="avg" data={data as Pt[]} height={height} padding={padding} />
  );
}
